...
export default function ChatInputForm({ activeChatId, apiKey, onMessageSent, API_URL, replyTo = null, onClearReply }) {
  ...
  return (
    <div className={`shrink-0 border-t safe-area-bottom ${theme === 'dark' ? 'border-[#1e222b] bg-[#0d1015]' : 'border-slate-200 bg-white'}`}>
      {replyTo && (
        <div className={`flex items-center gap-3 px-4 pt-3 ${theme === 'dark' ? 'bg-[#12151a]' : 'bg-slate-50'}`}>
          ...
          <button onClick={onClearReply} className={`p-2 rounded-lg ${theme === 'dark' ? 'text-slate-500 hover:text-white' : 'text-slate-400 hover:text-slate-700'}`}>
            <X className="w-4 h-4" />
          </button>
        </div>
      )}
      <form onSubmit={handleSend} className={`p-3 sm:p-4 ${theme === 'dark' ? 'bg-[#0d1015]' : 'bg-white'}`}>
        <div className={`flex items-end rounded-xl border p-1.5 sm:p-2 transition-colors ${theme === 'dark' ? 'bg-[#16191f] border-[#262931] focus-within:border-indigo-500/50' : 'bg-slate-50 border-slate-300 focus-within:border-indigo-400'}`}>
          <textarea 
            value={replyText}
            onChange={(e) => setReplyText(e.target.value)}
            onKeyDown={(e) => { if (e.key === 'Enter' && !e.shiftKey) { e.preventDefault(); handleSend(e); } }}
            placeholder={replyTo ? 'Write your reply...' : 'Type a message...'}
            className="flex-1 max-h-32 bg-transparent border-none focus:ring-0 text-sm sm:text-base p-2 resize-none outline-none dark:text-slate-200 text-slate-800 placeholder-slate-500"
            rows={1}
          />
          <button 
            type="submit" 
            disabled={sending || !replyText.trim()}
            className="w-11 h-11 mb-0.5 mr-0.5 rounded-xl bg-indigo-600 hover:bg-indigo-500 text-white disabled:opacity-50 transition-colors shrink-0 flex items-center justify-center"
          >
            {sending ? <LoaderCircle className="w-5 h-5 animate-spin" /> : <Send className="w-5 h-5 ml-0.5" />}
          </button>
        </div>
        <div className={`mt-2 text-center text-[10px] font-medium tracking-wide ${theme === 'dark' ? 'text-slate-600' : 'text-slate-400'} hidden sm:block`}>
