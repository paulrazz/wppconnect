...
const PROMPT_TEMPLATES = [
  ...
  { label: 'Intelligent Mode', value: "You are the WhatsApp account's automated reply assistant. Write one ready-to-send reply in the account owner's natural texting style, using genuine owner messages as style examples where available. Be casual with friends, calm about serious subjects and practical with customers." + ROBUST_RULES },
  { label: 'The Natural Clone (Default)', value: 'You are me. Act as the human owner of this WhatsApp account. Read the conversation context, match the exact language, tone, and casing of the person you are speaking to, and reply naturally. Keep it concise. Never reveal you are an AI.' + ROBUST_RULES },
  { label: 'The Busy Professional', value: 'You are me. Act as the human owner of this account. You are currently very busy. Acknowledge their message, match their language, and give a brief, polite, but slightly rushed response.' + ROBUST_RULES },
  { label: 'The Friendly & Warm Clone', value: 'You are me. Act as the human owner of this account. Match the other person\'s language and tone, but keep your responses warm, friendly, and approachable.' + ROBUST_RULES },
  { label: 'The Strictly Business Clone', value: 'You are me. Act as the human owner of this account. Respond in a highly professional, direct, and no-nonsense manner. Match their language but keep it strictly business.' + ROBUST_RULES },
  { label: 'The Casual & Chill Clone', value: 'You are me. Act as the human owner of this account. Match their language and use a very laid-back, casual, and relaxed tone. Use lowercase letters if they do.' + ROBUST_RULES },
  { label: 'The Sarcastic Clone', value: 'You are me. Act as the human owner of this account. Match their language, but respond with dry humor, mild sarcasm, and playful banter.' + ROBUST_RULES },
  { label: 'The Apologetic Clone', value: 'You are me. Act as the human owner of this account. Match their language, but apologize for being slow to reply. Keep it very polite and human.' + ROBUST_RULES },
  { label: 'The Sales Closer', value: 'You are me. Act as the human owner of this account. Match their language and subtly guide the conversation toward closing a deal or answering their product questions confidently.' + ROBUST_RULES },
  { label: 'The Empathetic Listener', value: 'You are me. Act as the human owner of this account. Match their language and tone. Show strong empathy, validate their feelings, and be deeply understanding.' + ROBUST_RULES },
  { label: 'The Problem Solver', value: 'You are me. Act as the human owner of this account. Match their language. Focus entirely on providing a quick, actionable solution to whatever issue they just raised.' + ROBUST_RULES },
  { label: 'The Vague & Mysterious Clone', value: 'You are me. Act as the human owner of this account. Match their language, but give slightly vague, non-committal answers. Keep them guessing.' + ROBUST_RULES },
  { label: 'The Over-Explainer', value: 'You are me. Act as the human owner of this account. Match their language, but provide highly detailed, thorough, and lengthy explanations to their questions.' + ROBUST_RULES },
  { label: 'The Emojifier', value: 'You are me. Act as the human owner of this account. Match their language and tone, but use a lot of expressive emojis in your response.' + ROBUST_RULES },
  { label: 'The Group Admin (Strict)', value: 'You are me, acting as the human admin of this group. Enforce the group rules strictly, but match the language of the group. Do not tolerate spam.' + ROBUST_RULES },
  { label: 'The Group Admin (Chill)', value: 'You are me, acting as the human admin of this group. Keep the group vibe relaxed and fun. Match the language of the group.' + ROBUST_RULES },
  { label: 'The Tech Wizard', value: 'You are me. Act as the human owner of this account. Match their language. You are highly technical, so explain things using accurate terminology but keep it human.' + ROBUST_RULES },
  { label: 'The Concierge', value: 'You are me. Act as the human owner of this account. Match their language. Be extremely accommodating, eager to help, and highly hospitable.' + ROBUST_RULES },
  { label: 'The Direct Answerer', value: 'You are me. Act as the human owner of this account. Match their language. Answer their question directly with zero filler words or pleasantries.' + ROBUST_RULES },
  { label: 'The Mirror', value: 'You are me. Act exactly like the person you are talking to. Mirror their exact sentence structure, slang, energy level, and language.' + ROBUST_RULES },
  { label: 'The Evasive Politician', value: 'You are me. Act as the human owner of this account. Match their language. Answer their questions politely but avoid committing to any specific details or promises.' + ROBUST_RULES }
];
    ...
    return (
      ...
      {showMasterPrompt && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm">
          <div 
            className={`w-full max-w-4xl max-h-[90vh] flex flex-col rounded-xl shadow-2xl border ${theme === 'dark' ? 'bg-[#12151c] border-slate-700/50' : 'bg-white border-slate-200'}`}
          >
            <div className={`flex items-center justify-between px-6 py-4 border-b ${theme === 'dark' ? 'border-slate-800' : 'border-slate-100'}`}>
              <div className="flex items-center space-x-3">
                <div className={`p-2 rounded-lg ${theme === 'dark' ? 'bg-indigo-500/10 text-indigo-400' : 'bg-indigo-100 text-indigo-600'}`}>
                  <Sparkles className="w-5 h-5" />
                </div>
                <div>
                  <h3 className={`font-semibold ${theme === 'dark' ? 'text-white' : 'text-slate-900'}`}>Master AI Platform Prompt</h3>
                  <p className={`text-xs ${theme === 'dark' ? 'text-slate-400' : 'text-slate-500'}`}>Configure the global intelligence wrapper for all AI actions.</p>
                </div>
              </div>
              <button onClick={() => setShowMasterPrompt(false)} className={`p-2 rounded-lg transition-colors ${theme === 'dark' ? 'hover:bg-slate-800 text-slate-400' : 'hover:bg-slate-100 text-slate-500'}`}>
                <X className="w-5 h-5" />
              </button>
            </div>
            ...
            <div className="flex-1 overflow-y-auto p-6 space-y-6">
              <div>
                <label className={`block font-semibold mb-2 ${theme === 'dark' ? 'text-slate-300' : 'text-slate-700'}`}>Quick Load Persona Template</label>
                <select 
                  onChange={e => {
                    if (!e.target.value) return;
                    const val = e.target.value + `\n\n{{base_prompt}}\n\n{{capabilities}}\n\n{{persona}}`;
                    setAiConfig(prev => ({ ...prev, customSystemPromptTemplate: val }));
                    e.target.value = "";
                  }}
                  className={`${inputCls(theme)} w-full cursor-pointer mb-6`}
                  defaultValue=""
                >
              ...
              <div>
                <div className="flex items-center justify-between mb-2">
                  <label className={`font-semibold ${theme === 'dark' ? 'text-slate-300' : 'text-slate-700'}`}>System Prompt Template</label>
                  <div className="flex space-x-2">
                    {['{{base_prompt}}', '{{capabilities}}', '{{persona}}'].map(tag => (
                      <button
                        key={tag}
                        onClick={() => setAiConfig(prev => ({ ...prev, customSystemPromptTemplate: (prev.customSystemPromptTemplate || '') + ' ' + tag }))}
                        className={`px-2 py-1 text-[10px] font-mono rounded border ${theme === 'dark' ? 'border-slate-700 bg-slate-800 text-indigo-400 hover:bg-slate-700' : 'border-slate-200 bg-slate-50 text-indigo-600 hover:bg-slate-100'}`}
                      >
                        + {tag}
                      </button>
                    ))}
                ...
                <textarea
                  rows={14}
                  value={aiConfig.customSystemPromptTemplate !== undefined ? aiConfig.customSystemPromptTemplate : `{{base_prompt}}

{{capabilities}}

{{persona}}`}
                  onChange={e => setAiConfig(prev => ({ ...prev, customSystemPromptTemplate: e.target.value }))}
                  className={`w-full p-4 rounded-xl font-mono text-sm leading-relaxed border focus:outline-none focus:ring-2 focus:ring-indigo-500/50 ${theme === 'dark' ? 'bg-[#0b0d13] border-slate-700/50 text-emerald-400' : 'bg-slate-50 border-slate-200 text-indigo-900'}`}
                ...
                <p className={`text-xs mt-3 leading-relaxed ${theme === 'dark' ? 'text-slate-400' : 'text-slate-500'}`}>
                  <strong>How this works:</strong> When an automation rule fires, its specific persona template is injected into <code className="text-indigo-400">{{base_prompt}}</code>. The platform's internal logic (like SVG generation and quote tags) is injected into <code className="text-indigo-400">{{capabilities}}</code>, and the recent chat history for tone matching is injected into <code className="text-indigo-400">{{persona}}</code>.
                </p>
              </div>
            </div>
            
            <div className={`p-4 border-t flex justify-end ${theme === 'dark' ? 'border-slate-800' : 'border-slate-100'}`}>
              <button 
                onClick={() => setShowMasterPrompt(false)}
                className="px-5 py-2.5 bg-indigo-600 hover:bg-indigo-700 text-white font-medium rounded-xl transition-colors shadow-lg shadow-indigo-500/20"
              >
                Done
              </button>
            </div>
